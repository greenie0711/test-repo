...
```{r}
num_secs <- 65
var_seconds <- paste(num_secs, "seconds is", num_secs%/%60, "minutes and", num_secs%%60, "seconds")

if (num_secs ==1){
  var_seconds <- gsub("seconds is", "second is", var_seconds)}
if (num_secs%%60 == 1){
  var_seconds <- gsub("seconds", "second", var_seconds)}
if (num_secs%/%60 == 1){
  var_seconds <- gsub("minutes and", "minute and", var_seconds)}
print(var_seconds)
...
for (i in 2:(is_prime - 1)){
  if (is_prime%%i==0){
    cat(is_prime, "is not a prime number")
    prime <- FALSE
    break
  }
}
if (prime == TRUE){
  cat(is_prime, "is a prime number")
}
...
cat(first_to_exceed, "squared is", (first_to_exceed^2), "and is the first number whose square exceeds", limit)
