...
sprintf("The largest win spread is %d, when the Bears scored %d points and the opponent scored %d points", spread, bears_score, opp_score)
```
...
```{r}
updated_score <- matrix(c(37,9, 17, 17), 2, 2)
game_score <- rbind(game_score, updated_score)
print(game_score)
```
...
for (i in 1:nrow(game_score)){
  if ((game_score[i,1] - game_score[i,2]) > 0){
    print(paste(i, "WIN"))
  }else if(game_score[i,1] - game_score[i, 2] < 0){
    print(paste(i, "LOSS"))
  }
}
...
```{r}
first_win <- which(game_score[ ,3]==TRUE)
losses_until_win <- min(first_win) - 1
print(losses_until_win)
```
...
```{r}
number_of_factors <- function(value){
  factor_count = 0
  for (i in 1:value){
    if (value%%i == 0){
...
```{r}
area <- function(length = 48, width = 48){
  inches <- (length*width)
  feet <- ((length*width)/144)
  return(c(inches,feet))
  }
rectangle1 = (area(102,60))
print(sprintf("The area in square inches is %.1f", rectangle1[1]))
print(sprintf("The area in square feet is %.1f", rectangle1[2]))
...
```{r}
vector_check <- function(value, vector){
  in_list <- FALSE
  for (i in vector){
    if (i == value){
      in_list <- TRUE
...
```{r}
positive_matrix <- function(input){
  input[input<0] <- NA
  return(input)
  }
positive_matrix(x)
